...
// Room attributes
const sprintCount = ref(manageRoom.number_of_sprints);
const buildPhaseDuration = ref(manageRoom.build_phase_duration);
const roomName = ref(manageRoom.name);
const meetingDuration = ref(manageRoom.planning_duration);
...
const editRoomFunction = async () => {
  ...
  loading.value = true;
  try {
    const response = await client(`/api/rooms/${manageRoom.id}`, {
      method: "PUT",
      body: {
        name: roomName.value,
    ...
    toast.add({
      title: useNuxtApp().$i18n.t("edit_room.room_editted"),
      icon: "clarity:success-line",
      variant: "success",
    });
    navigateTo(
      localeRoute({ name: "rooms-id-parent", params: { id: manageRoom.id } }),
    );
  } catch (error) {
    toast.add({
      title: useNuxtApp().$i18n.t("edit_room.edit_error"),
    ...
    <div class="mt-12 flex h-full flex-col gap-10">
      ...
      <div class="mx-40 flex justify-center gap-28">
        ...
        <div class="flex flex-col items-center gap-3">
          <div class="flex flex-col items-center gap-3">
            <label for="sprints" class="block text-lg font-medium">{{
              $t("edit_room.build_phase_duration")
            }}</label>
            <Chooser
          ...
          >
            <p>{{ $t("edit_room.total_duration") }}:</p>
            <span
              class="flex justify-center text-sc-orange"
              v-if="totalDuration > 0"
              >&nbsp;{{
                totalDuration + " " + $t("edit_room.minute", totalDuration)
              }}</span
            >
          </div>
        </div>
