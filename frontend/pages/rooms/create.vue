...
<template>
  <UModal v-model="isOpen" prevent-close :ui="{
    ...
    <SvgRoomCreation class="absolute bottom-16" :fontControlled="false" filled />
    ...
    <div class="mt-12 flex h-full flex-col gap-10">
      ...
      <div class="mx-40 flex justify-center gap-28">
        ...
        <div class="flex flex-col items-center gap-3">
          ...
          <div class="text-md relative left-6 top-[4em] flex h-9 w-56 items-center pl-2">
            <p>{{ $t("create_room.total_duration") }}:</p>
            <span class="flex justify-center text-sc-orange" v-if="totalDuration > 0">&nbsp;{{
    ...
    <div class="flex justify-center">
      ...
      <UButton @click="createRoomFunction"
        class="absolute bottom-8 right-10 h-[2.9em] w-44 justify-center rounded-lg disabled:opacity-100 disabled:bg-sc-black-400 text-2xl disabled:text-white bg-sc-green text-sc-black hover:bg-sc-green-400 disabled:pointer-events-none"
        :disabled="!allSet">{{ $t("create_room.create") }}</UButton>
    </div>
